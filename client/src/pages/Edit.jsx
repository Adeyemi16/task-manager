...
import { editTask, specificTask } from "../config/api";
...
export default function NewTask() {
  ...
  const { error, data, loading, setLoading, setError } = useFetchData(
    specificTask,
    taskId,
  ...
  const {
    register,
    handleSubmit,
    setValue, // Add setValue from react-hook-form to set form field values
    formState: { errors },
  } = useForm();
  ...
  useEffect(() => {
    ...
    // Set form field values when data is available
    if (data) {
  ...
  const onSubmitHandler = async (data) => {
    ...
    try {
      setLoading(true);
      const res = await editTask(taskId, updatedTask, currentUser.access_token);
      if (res.status === 201) {
        ...
        >
          ...
          <div className="flex justify-between" style={{ width: "100%" }}>
            <div>
              ...
              >
                <option value="Urgent" style={{ cursor: "pointer" }}>
                  Urgent
                </option>
                <option value="Important" style={{ cursor: "pointer" }}>
                  Important
                </option>
              </select>
              {errors.tag && (
